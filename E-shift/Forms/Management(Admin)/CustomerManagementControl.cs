...
namespace E_shift.Forms
{
    ...
    {
        private DataTable _allCustomersTable; // cache all data here
        ...
        public CustomerManagementControl()
        {
            ...
            SearchHelper.AttachSearch(txtSearch, cmbSearchBy, (col, keyword) =>
            {
                ...
                {
                    // No keyword: show all
                    dgvCustomers.DataSource = _allCustomersTable;
                    return;
                }
                ...
                if (!columnMap.TryGetValue(col ?? "", out string dtCol))
                    return;

                if (dtCol == "CustomerID" && !int.TryParse(keyword, out _))
                    return;

                if (_allCustomersTable != null)
                {
                    DataTable filtered = SearchHelper.LinearSearch(_allCustomersTable, dtCol, keyword);
        ...
        private void LoadCustomerData()
        {
            try
            {
                // Load all customers once, cache
                string query = "SELECT CustomerID, FullName, Phone, Email, Address FROM Customer ORDER BY CustomerID DESC";
                ...
                using (SqlConnection conn = Database.GetConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        _allCustomersTable = dt; // cache full data
        ...
        {
            AddUserForm addCustomerForm = new AddUserForm();
                        ...
                        using (SqlConnection conn = Database.GetConnection())
                        {
                            ...
                            using (SqlTransaction transaction = conn.BeginTransaction())
                            {
                                ...
                                catch (SqlException sqlEx) when (sqlEx.Number == 547) // FK violation
                                {
                                    ...
                                    MessageBox.Show(
                                        "Cannot delete this customer because there are related records in other tables. " +
                                        "Please delete related jobs or  loads first.",
                                        "Delete Error",
