...
namespace E_shift.Forms
{
    ...
    {
        private DataTable _allContainersTable; // cache all data here
        ...
        public ContainerManagementControl()
        {
            ...
            SearchHelper.AttachSearch(txtSearch, cmbSearchBy, (col, keyword) =>
            {
                ...
                {
                    dgvContainers.DataSource = _allContainersTable;
                    return;
                }
                ...
                if (!map.TryGetValue(col ?? "", out string dtCol))
                    return;

                if (dtCol == "ContainerID" && !int.TryParse(keyword, out _))
                    return;

                if (_allContainersTable != null)
                {
                    DataTable filtered = SearchHelper.LinearSearch(_allContainersTable, dtCol, keyword);
                    dgvContainers.DataSource = filtered;
                }
            });
        ...
        private void LoadContainerData()
        {
            ...
            try
            {
                ...
                // Cache the full data table
                _allContainersTable = dt;
