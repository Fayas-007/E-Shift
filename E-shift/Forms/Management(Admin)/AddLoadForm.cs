...
namespace E_shift.Forms
{
    public partial class AddLoadForm : Form
    {
        ...
        private void BtnAddLoad_Click(object sender, EventArgs e)
        {
            ...
            // Check if job is cancelled before proceeding
            if (IsJobCancelled(jobId))
            {
                MessageBox.Show("Cannot add load to a cancelled job.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            try
            {
                using (var conn = Database.GetConnection())
                {
                    ...
                    using (var transaction = conn.BeginTransaction())
                    {
                        ...
                        {
                            string insertQuery = @"
                                INSERT INTO Load (JobID, Weight, StartDate, EndDate, CreatedAt, TransportUnitID)
                                VALUES (@JobID, @Weight, @StartDate, @EndDate, @CreatedAt, @TransportUnitID);
                            ...
                            transaction.Commit();
                            JobStatusHelper.SyncJobStatus(); // Sync job status accordingly
