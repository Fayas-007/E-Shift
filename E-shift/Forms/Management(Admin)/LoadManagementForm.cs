...
namespace E_shift.Forms
{
    ...
    {
        // Cache all loads in memory for search/filtering
        private DataTable _allLoads;
        ...
        public LoadManagementControl()
        {
            ...
            // Attach search using in-memory filtering
            SearchHelper.AttachSearch(txtSearch, cmbSearchBy, (col, keyword) =>
            {
                LoadLoadData(col, keyword);
            });
        ...
        private DataTable GetLoadDataTable()
        {
            string sql = @"
                SELECT 
                    l.LoadID,
                    ...
                    l.Status, 
                    l.CreatedAt,
                    CONVERT(varchar, l.StartDate, 23) AS StartDate,
                    CONVERT(varchar, l.EndDate, 23) AS EndDate,
        ...
        private void LoadLoadData(string columnFilter = null, string keyword = null)
        {
            ...
            {
                if (_allLoads == null)
                    _allLoads = GetLoadDataTable();  // Load all data once and cache
                ...
                {
                    // Map combo box visible columns to actual DataTable column names
                    ...
                    if (!map.TryGetValue(columnFilter, out string dtColumn))
                        dtToBind = _allLoads; // fallback to full data
                    else
                        dtToBind = SearchHelper.LinearSearch(_allLoads, dtColumn, keyword);
                }
            ...
            else if (columnName == "Delete")
            {
                ...
                if (confirmResult == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        _allLoads = null; // reset cache after delete
                        LoadLoadData();
                    }
                    catch (Exception ex)
                    {
        ...
        private void BtnAddLoad_Click(object sender, EventArgs e)
        {
            ...
            {
                _allLoads = null; // reset cache on new load add
                LoadLoadData();
            }
        }
        ...
        {
            var dt = _allLoads ?? GetLoadDataTable();  // get cached or fresh data
            LoadReportViewer viewer = new LoadReportViewer(dt);
            viewer.ShowDialog();
        }
    }
}
